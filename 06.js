var masterInput = `turn off 660,55 through 986,197
...
function createCommandPart1(commandText, fromX, fromY, toX, toY) {
    var action = commandText == 'turn off' ? _ => 0 :
        commandText == 'turn on' ? _ => 1 :
            x => x ? 0 : 1;
    return {
        action,
        fromX,
...
function createCommandPart2(commandText, fromX, fromY, toX, toY) {
    var action = commandText == 'turn off' ? x => { x = x - 1; return x < 0 ? 0 : x; } :
        commandText == 'turn on' ? x => x + 1 :
            x => x + 2;
    return {
        action,
        fromX,
...
function execCommand(command, grid) {
    for (var x = command.fromX; x <= command.toX; x++) {
        for (var y = command.fromY; y <= command.toY; y++) {
            grid[x][y] = command.action(grid[x][y]);
        }
    }
}

function createCommands(input, commandCreator) {
    var exp = /((turn off)|(toggle)|(turn on)) (\d*),(\d*) through (\d*),(\d*)/g;
    var line;
    var commands = [];
    while ((line = exp.exec(input)) !== null) {
        var command = commandCreator(line[1], parseInt(line[5]), parseInt(line[6]), parseInt(line[7]), parseInt(line[8]));
...
function run(input, commandCreator) {
    var commands = createCommands(input, commandCreator);
    var grid = createGrid();
    commands.forEach(c => execCommand(c, grid));
    return grid.reduce((accR, row) => accR + row.reduce((accC, cell) => accC + cell, 0), 0);
}

function parse(input) {
    //400410
    console.log(run(input, createCommandPart1));
    //15343601
    console.log(run(input, createCommandPart2));
}
parse(masterInput);