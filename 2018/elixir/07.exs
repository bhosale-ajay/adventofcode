...
defmodule D07 do
  def find_root_nodes([graph, dep]) do
    graph
    |> Map.keys()
  ...
  def determine_order([graph, dep]) do
    traverse_nodes(find_root_nodes([graph, dep]), graph, dep, "")
  end

  def count_steps([graph, dep], worker_count, base) do
    process_steps(find_root_nodes([graph, dep]), [], graph, dep, 0, worker_count, base)
  end
...
"CABDFE" = D07.determine_order(test_requirements)
"BDHNEGOLQASVWYPXUMZJIKRTFC" = D07.determine_order(puzzle_requirements)
15 = D07.count_steps(test_requirements, 2, 0)
1107 = D07.count_steps(puzzle_requirements, 5, 60)
