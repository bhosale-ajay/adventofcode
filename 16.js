var mainInput = `Sue 1: cars: 9, akitas: 3, goldfish: 0
...
function comparer1(memoryData, scanData){
    var matchCount = 0;
    var propCount = 0;
    for(var prop in memoryData){
        propCount = propCount + 1;
        if(memoryData[prop] === scanData[prop]){
            matchCount = matchCount + 1;
        }
    }    
    return matchCount > 0 && propCount == matchCount;
}

function comparer2(memoryData, scanData){
    var matchCount = 0;
    var propCount = 0;
...
function find(memories, scanData, comparer){
    var expression = /Sue (\d+): ((?:[a-z]*:\s(?:\d+),?\s*)*)/g;
    var counter = 1;
    var memory, memoryData;
    while ((memory = expression.exec(memories)) !== null) {
        eval("memoryData = {" + memory[2] + "};");
        if(comparer(memoryData, scanData)){
            return counter;   
        }
        counter = counter + 1;
    }
...
var scanData = {children: 3, cats: 7, samoyeds: 2, pomeranians: 3, akitas: 0, 
                vizslas: 0, goldfish: 5, trees: 3, cars: 2, perfumes: 1};

//373
console.log(find(mainInput, scanData, comparer1));
//260
console.log(find(mainInput, scanData, comparer2));